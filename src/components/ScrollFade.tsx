...
export function ScrollFade({ children, className = "" }: ScrollFadeProps) {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      // How far the element center is from viewport center, normalized 0–1
      const center = rect.top + rect.height / 2;
      const distFromCenter = Math.abs(center - vh / 2);
      const maxDist = vh / 2 + rect.height / 2;

      // 1 when centered, 0 when fully off
      const ratio = Math.max(0, 1 - distFromCenter / maxDist);

      // Ease curve for smoother feel
      const eased = ratio < 0.3 ? ratio / 0.3 : 1; // quick fade-in in first 30%
      const fadeOut = ratio < 0.15 ? ratio / 0.15 : 1; // fade-out at edges
