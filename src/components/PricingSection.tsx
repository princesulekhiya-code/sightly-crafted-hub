            ...
            {plans.map((plan) => {
              ...
              return (
                <div key={plan.id} className={`glass-card rounded-2xl p-8 relative ${plan.popular ? "border border-primary/30 scale-[1.02]" : ""}`}>
                  ...
                  <Link to={plan.href} className={`block text-center py-2.5 rounded-full text-sm font-semibold transition-all ${plan.popular ? "bg-primary text-primary-foreground hover:opacity-90" : "bg-secondary text-foreground hover:bg-accent"}`}>
                    {plan.cta}
                  </Link>
